...
export default function TerminalLogs({ logs }) {
  ...
  const getAgentColor = (agent) => {
    const map = {
      'agent-recon': '#22D3EE',    // primary
      'agent-strategy': '#7C3AED', // secondary
      'agent-exploit': '#FF4D6D',  // danger
      'agent-auth': '#FACC15',     // warning
      'agent-risk': '#22C55E'      // success
    };
  ...
  return (
    <div className="terminal-block h-full flex flex-col overflow-hidden group">
      ...
      {/* Header Bar */}
      <div className="flex items-center justify-between mb-16 border-b border-border-dim pb-8 opacity-60">
        <div className="flex items-center gap-8">
          <div className="w-6 h-6 rounded-full bg-primary animate-pulse" />
          <span className="label-caps !text-[9px]">Neural Link: Online</span>
        </div>
